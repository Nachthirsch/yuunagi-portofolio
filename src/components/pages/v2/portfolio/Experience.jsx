        ...
        <div className="space-y-16 md:space-y-32 mb-32">
          {experiences.map((exp, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: index * 0.2 }} className="relative">
              {/* Large Year Display */}
              <div
                className="absolute -left-4 sm:block hidden top-0 text-[8rem] md:text-[10rem] font-bold text-neutral-50 select-none
              ...
              {/* Type Indicator */}
              <div
                className="absolute -right-4 sm:block hidden top-0 text-[1.5rem] font-medium text-neutral-400/50 select-none
                  ...
                  <div className="space-y-4">
                    {exp.description.map((item, idx) => (
                      <motion.div key={idx} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + idx * 0.1 }} className="flex items-start gap-2 md:gap-4 group">
                        <ArrowRight
                          size={16}
                          className="flex-shrink-0 mt-1.5 text-neutral-300 group-hover:text-blue-500 

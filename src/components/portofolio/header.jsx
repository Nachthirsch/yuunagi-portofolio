...
import { FaInstagram, FaGithub, FaLinkedin, FaSoundcloud,  FaLastfmSquare } from "react-icons/fa";
    ...
    const fetchRecentTracks = async () => {
      try {
        ...
        const response = await fetch(
          `https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=yuunaagi&api_key=${apiKey}&format=json&limit=1`
        );
    ...
    <header className="relative h-screen font-Hanken tracking-wider overflow-hidden bg-neutral-900">
      ...
      <div className="relative h-full flex flex-col justify-center px-4 sm:px-6 md:px-12 z-10">
        ...
        <motion.div 
          ...
          className="mt-6 mb-6"
        >
          <div className="bg-neutral-800 py-3 px-4 rounded-sm border-2 border-black shadow-[4px_4px_0px_rgb(0,0,0)] max-w-sm">
            <div className="flex items-center mb-4">
              <FaLastfmSquare className="text-red-700 mr-2 text-sm" />
            ...
            {isLoading ? (
              <div className="py-1 text-neutral-300 text-xs">Loading...</div>
            ) : lastTrack ? (
              <div className="flex items-start">
                {lastTrack.image && lastTrack.image.length > 0 ? (
                  <div className="mr-2 flex-shrink-0 w-10 h-10 bg-neutral-700 border border-black overflow-hidden">
                    <img 
                      src={lastTrack.image[2]?.['#text'] || lastTrack.image[1]?.['#text'] || lastTrack.image[3]?.['#text']} 
                  ...
                  <div className="mr-2 flex-shrink-0 w-10 h-10 bg-neutral-700 border border-black flex items-center justify-center">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-neutral-100 text-xs mt-0.5">
                    by {lastTrack.artist['#text']}
                  </p>
                </div>
              </div>
            ) : (
              <p className="text-neutral-300 text-xs py-1">No recent tracks found</p>
            )}
          </div>
        </motion.div>

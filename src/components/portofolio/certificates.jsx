  ...
  return (
    <section className="py-16 px-4 sm:px-8 md:px-16 bg-neutral-900 font-Hanken tracking-wider">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-8 sm:mb-16 relative">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={toggleViewMode} className="flex items-center gap-2 py-2 px-4 bg-neutral-800 border-3 border-black shadow-[4px_4px_0px_rgba(0,0,0,0.8)] hover:-translate-y-1 transition-all duration-200 rotate-1" aria-label={`Switch to ${viewMode === "slider" ? "list" : "slider"} view`}>
              {viewMode === "slider" ? (
                <>
          ...
          <motion.div className="w-32 h-3 bg-neutral-300 mt-5 -rotate-1 border-3 border-black shadow-[5px_5px_0px_rgba(0,0,0,0.8)]" initial={{ width: 0 }} whileInView={{ width: "10rem" }} transition={{ duration: 0.8, delay: 0.3 }} />
        </motion.div>
        ...
        {viewMode === "slider" ? (
          ...
          <div className="relative cert-slider-container">
            ...
            <div className="slider-navigation flex justify-between absolute top-1/2 -translate-y-1/2 w-full z-30 px-2 sm:px-6 pointer-events-none">
              <button onClick={() => swiperRef.current?.slidePrev()} className="nav-btn-prev bg-neutral-800 p-2 sm:p-3 border-3 border-black shadow-[4px_4px_0px_rgba(0,0,0,0.8)] hover:-translate-y-1 transition-all duration-200 rotate-2 pointer-events-auto" aria-label="Previous slide">
                <IoIosArrowBack className="text-lg sm:text-xl text-neutral-300" />
              </button>
              <button onClick={() => swiperRef.current?.slideNext()} className="nav-btn-next bg-neutral-800 p-2 sm:p-3 border-3 border-black shadow-[4px_4px_0px_rgba(0,0,0,0.8)] hover:-translate-y-1 transition-all duration-200 -rotate-2 pointer-events-auto" aria-label="Next slide">
                    ...
                    <div className={`flex gap-5 h-full relative z-10 p-6 ${index % 3 === 0 ? "-rotate-2" : index % 3 === 1 ? "rotate-2" : "rotate-0"}`}>
                      ...
                      <div className="flex flex-col h-full w-full">
                        ...
                        <div className="flex-grow">
                          <h3 className="text-lg font-extrabold text-neutral-300 line-clamp-2 group-hover:line-clamp-none transition-all uppercase text-shadow-small">{cert.title}</h3>
                          <p className="text-sm text-neutral-400 mt-2 line-clamp-1 group-hover:line-clamp-none transition-all font-bold">{cert.issuer}</p>
                          <p className="text-xs text-neutral-500 mt-3 inline-block px-3 py-1 border-2 border-black shadow-[3px_3px_0px_rgba(0,0,0,0.8)] bg-neutral-700 font-bold rotate-1">{cert.date}</p>
                        </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="cert-list-container">
            <div className="border-4 border-black shadow-[8px_8px_0px_rgba(0,0,0,0.8)] bg-neutral-800 overflow-hidden">
              ...
              <div className="max-h-[500px] overflow-y-auto cert-list-scrollbar">
                {certificates.map((cert, index) => (
                  <motion.div key={index} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: index * 0.05 }} className={`grid grid-cols-12 gap-4 p-4 items-center border-b-2 border-neutral-700 hover:bg-neutral-700/30 transition-colors duration-200 ${index % 2 === 0 ? "bg-neutral-800" : "bg-neutral-800/70"}`}>
                    <div className="col-span-7 sm:col-span-8 flex items-center gap-3">
                      ...
                      <h3 className="text-sm font-bold text-neutral-300 line-clamp-1">{cert.title}</h3>
                    </div>
                    <div className="col-span-3 sm:col-span-2 text-center">
                      <p className="text-xs text-neutral-400 inline-block px-2 py-1 border-2 border-black shadow-[2px_2px_0px_rgba(0,0,0,0.8)] bg-neutral-700 font-bold">{cert.date}</p>
                    </div>
                    <div className="col-span-2 text-center">
                      {cert.credential && (
                        <a
